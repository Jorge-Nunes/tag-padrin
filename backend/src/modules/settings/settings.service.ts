...
export class SettingsService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (!settings) {
      ...
      const updates: any = {};
      if (!settings.brgpsBaseUrl)
        updates.brgpsBaseUrl = this.configService.get('BRGPS_BASE_URL') || '';
      if (!settings.brgpsToken)
        updates.brgpsToken = this.configService.get('BRGPS_API_TOKEN') || '';
