...
async function main() {
  ...
  // Use a very wide timeframe just to be sure
  // From 2026-03-01 to 2026-03-20
  const timeFrom = Math.floor(new Date('2026-03-01T00:00:00Z').getTime() / 1000);
  const timeTo = Math.floor(new Date('2026-03-20T00:00:00Z').getTime() / 1000);
  ...
  await attempt(`${user.brgpsBaseUrl}/tag/history?id=${id}&TimeFrom=${timeFrom}&TimeTo=${timeTo}`);
  await attempt(`${user.brgpsBaseUrl}/tag/history?timeFrom=${timeFrom}&timeTo=${timeTo}&id=${id}`);
}
